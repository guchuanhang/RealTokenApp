...
public class ProxyHandler implements InvocationHandler {
    ...
    private final static int REFRESH_TOKEN_VALID_TIME = 100;
    ...
    private Observable<?> refreshTokenWhenTokenInvalid() {
        synchronized (ProxyHandler.class) {
            ...
            if (new Date().getTime() - tokenChangedTime < REFRESH_TOKEN_VALID_TIME) {
                ...
                String refreshToken = ServerConfig.instance.tokenBean.refreshTokenStr;
                RetrofitService
                        .getRetrofitBuilder(mBaseUrl)
                        .build()
                        .create(CommonService.class)
                        .refreshToken(refreshToken)
                        .subscribe(new DefaultObserver<TokenBean>() {
                            @Override
